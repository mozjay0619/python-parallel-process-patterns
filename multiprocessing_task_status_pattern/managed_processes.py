...
class WorkerProcess(Process):
    def __init__(
        self, shared_results_dict, shared_status_dict, shared_error_dict, task, timeout
    ):
...
class ManagedProcesses:
    def __init__(
        self, n_procs=4, verbose=False, timeout=60 * 60, n_retries=3, *args, **kwargs
    ):
    ...
    def results(self, show_progress):
        ...
        while len(self.pending_tasks) > 0 or len(self.tasks) > 0:
                ...
                for _ in range(len(self.pending_tasks)):
                    ...
                    if pending_task_idx in self.failed_tasks_idx:
                        ...
                        if fail_count < self.n_retries:
                            ...
                            if self.verbose:
                                print(
                                    "    pending --> tasks: {}".format(pending_task_idx)
                                )
                            ...
                            warning_msg = "task id {} failed due to: {}".format(
                                pending_task_idx,
                                self.shared_error_dict[pending_task_idx],
                            )
                    ...
                    # premature and sudden death of Process
                        ...
                        if self.verbose:
                            print(
                                "    Process terminated abruptly while task {} pending".format(
                                    pending_task_idx
                                )
                            )
                            print("    pending --> tasks")
            ...
            if show_progress:
                ...
                cur_len = int(
                    len(self.successful_tasks_idx) + len(self.failed_tasks_idx)
                )
            ...
            if self.verbose:
                ...
                print(
                    "number of outstanding tasks: {} ( {} )".format(
                        len(self.tasks), self.tasks
                    )
                )
                print("number of running procs: {}".format(len(self.subprocs)))
                print()
