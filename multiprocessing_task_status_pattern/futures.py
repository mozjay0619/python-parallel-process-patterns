...
class Futures:
    def __init__(
        ...
        timeout=60 * 60,
        n_retries=3,
        parallelism_mode="managed.processes",
    ):
        ...
        if self.parallelism_mode == "managed.processes":
            self.core = ManagedProcesses(
                n_procs=n_procs, verbose=verbose, timeout=timeout, n_retries=n_retries
            )
    ...
    def globalize(self, local_var, global_name):
        ...
        if self.parallelism_mode == "managed.processes":
            ...
            raise Exception(
                "[globalize] method only supported for parallelism_mode: managed.processes"
            )
    ...
    def errors(self):
        ...
        if self.parallelism_mode == "managed.processes":
            ...
            raise Exception(
                "[errors] method only supported for parallelism_mode: managed.processes"
            )
